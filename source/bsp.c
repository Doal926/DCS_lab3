...
void GPIOconfig(void){
 // volatile unsigned int i; // in case of while loop usage
  ...
  WDTCTL = WDTHOLD + WDTPW;		// Stop WDT
    ...
    // Timer B1 config
    TimerB1_DIR |= 0x04;          // Set P2.2 to output direction from timer to buzzer
    TimerB1_SEL |= 0x04;          // Set P2.2 to output direction from timer to buzzer
...
//  DMA1CTL = DMADT_1+DMAEN;                    //channel 1 = block transfer
 // DMA0DA = (void (*) ()) &TBCCR0;
    DMA0CTL |= DMADT_2 + DMASRCINCR_3 + DMADSTINCR_3 + DMAIE + DMAREQ + DMAEN; // Enable DMA0, block transfer
